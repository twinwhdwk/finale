...
- **현재 핵심 방향: `note_recognition/` 패키지 — Audiveris를 쓰지 않는 자체 OpenCV
  ...
  **[방향 전환] step_diff — 렌더링/음가 인식 없는 채보 오류 탐지** (`step_diff.py`):
  자체 OMR의 잔여 노이즈(~600건)로는 실제 채보 오류(악보당 0~10건) 탐지가
  불가능하다는 판단에 따라, 경량 비교기로 방향 전환:
  - PDF 검출 음표머리의 diatonic step 시퀀스 ↔ MXL step 시퀀스를
    Needleman-Wunsch 전역 정렬 → 불일치가 몰린 마디만 "의심 마디" 보고
  - 음가 인식 미사용(OMR 최대 약점 회피), 임시표 무시, barline 분할 불필요
  - 파트 자동 선택(정렬 점수 최대), 조옮김/옥타브 offset(-7~+7) 자동 흡수
  - `python step_diff.py <pdf> <mxl> [part]` CLI
  실측: 태양F 93% 일치·강한 의심 4마디(+약한 4), 꿈꾸지 94%·강 6(+약 5).
  개선 이력: x_start를 barline-12gap으로 확대(첫 음/pickup 잘림 방지,
  조표 오검출은 정렬이 잉여로 흡수), |step차|=1은 반올림 경계로 보고
  '약한 의심'으로 등급화(정렬 점수 NEAR=0).
  주입 오류 탐지 13/15, 12/15 — 미탐 2~3건은 pdf 잉여 밀집 구간의
  정렬 국소 우회가 원인(엄격 정렬로도 동일).
  [기각된 시도] 시스템 그룹핑으로 파트0 오선만 추출해 잉여 축소:
  꿈꾸지 p2 그룹핑 실패(내부/간 gap 동일)로 페이지 간 오선 구성이
  뒤섞여 오히려 악화(강 6→18). 전체 오선 + 정렬 흡수가 더 강건.
